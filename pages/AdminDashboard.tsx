    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto min-h-screen">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6 text-left">
        <div>
          <div className="flex items-center gap-4 mb-2">
            ...
            <div className="flex items-center gap-2">
              <Link to="/" className="p-3 bg-white border rounded-2xl text-gray-400 hover:text-[var(--primary)] shadow-sm transition-all flex items-center gap-2 font-bold text-sm">
                <Home size={18} /> Student Site
              </Link>
              <button 
                onClick={logout}
                className="p-3 bg-red-50 border border-red-100 rounded-2xl text-red-400 hover:bg-red-500 hover:text-white shadow-sm transition-all flex items-center gap-2 font-bold text-sm"
              >
          ...
          <p className="text-gray-500 font-medium text-xl">
            Welcome back, <span className="text-gray-900 font-bold">{auth.label}</span>. Manage training assets and access.
          </p>
        ...
        <div className="flex gap-4">
          <button 
            ...
            className={`px-10 py-6 rounded-[2.5rem] flex items-center gap-4 font-black text-xl transition-all shadow-xl active:scale-95 border-4 ${showCodesTab ? 'bg-gray-900 text-white border-gray-900' : 'bg-white text-gray-900 border-gray-100 hover:border-[var(--primary)]'}`}
          >
            <Key size={28} /> {showCodesTab ? 'Close Access Codes' : 'Manage Access'}
          </button>
          {!showCodesTab && (
            <Link
              to="/create"
      ...
      {showCodesTab ? (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
           {/* New Code Form */}
           <div className="bg-white rounded-[3rem] p-12 shadow-xl border border-gray-100 text-left">
              <h2 className="text-3xl font-black text-gray-900 tracking-tighter mb-8 flex items-center gap-3">
                 <Key className="text-[var(--primary)]" size={32} /> Generate New Access Code
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                 <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-2">Team Label</label>
                    <input 
                      type="text" 
                      placeholder="e.g. Hospitality Team" 
                      value={newLabel}
                      onChange={e => setNewLabel(e.target.value)}
                      className="w-full p-4 bg-gray-50 border-2 border-gray-100 rounded-2xl font-bold outline-none focus:border-[var(--primary)]"
                    />
                 </div>
                 <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-2">Unique Code</label>
                    <input 
                      type="text" 
                      placeholder="ICPH2026-CODE" 
                      value={newCode}
                      onChange={e => setNewCode(e.target.value)}
                      className="w-full p-4 bg-gray-50 border-2 border-gray-100 rounded-2xl font-black uppercase tracking-widest outline-none focus:border-[var(--primary)]"
                    />
                 </div>
                 <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-2">Access Level</label>
                    <select 
                      value={newRole}
                      onChange={e => setNewRole(e.target.value as UserRole)}
                      className="w-full p-4 bg-gray-50 border-2 border-gray-100 rounded-2xl font-bold outline-none focus:border-[var(--primary)] cursor-pointer"
                    >
                 ...
                 <button 
                  ...
                  className="bg-gray-900 text-white p-5 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-black transition-all active:scale-95 disabled:opacity-30"
                 >
                    Create Access Code
                 </button>
              </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {accessCodes.sort((a,b) => b.createdAt - a.createdAt).map(ac => (
                <div key={ac.id} className="bg-white p-8 rounded-[2.5rem] border-2 border-gray-100 shadow-sm flex flex-col relative group overflow-hidden">
                   <div className={`absolute top-0 right-0 p-4 rounded-bl-3xl font-black text-[10px] uppercase tracking-widest ${ac.role === 'admin' ? 'bg-red-50 text-red-500' : 'bg-green-50 text-green-500'}`}>
                      {ac.role}
                   </div>
                   <div className="mb-6">
                      <h4 className="font-black text-2xl text-gray-900 mb-1">{ac.label}</h4>
                      <p className="text-gray-400 text-xs font-medium">Created {new Date(ac.createdAt).toLocaleDateString()}</p>
                   </div>
                   <div className="bg-gray-50 p-6 rounded-2xl mb-8 border border-gray-100">
                      <code className="text-2xl font-black text-[var(--primary)] tracking-[0.2em]">{ac.code}</code>
                   </div>
                   <button 
                    onClick={() => { if(confirm('Permanently revoke this access code?')) deleteAccessCode(ac.id); }}
                    className="w-full py-4 text-red-500 font-black text-xs uppercase tracking-widest bg-red-50 rounded-xl hover:bg-red-500 hover:text-white transition-all border border-red-100"
                   >
                      Revoke Access
                   </button>
                </div>
